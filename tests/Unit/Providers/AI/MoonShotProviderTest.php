...
use ErdiKoroglu\AIContentGenerator\Providers\AI\MoonShotProvider;
...
test('getName returns moonshot', function () {
    $provider = new MoonShotProvider($this->config);
    
    expect($provider->getName())->toBe('moonshot');
});
...
test('generateContent returns content on successful response', function () {
    $mock = new MockHandler([
        new Response(200, [], json_encode([
            'choices' => [
                [
                    'message' => [
                        'content' => 'Generated content from MoonShot AI',
                    ],
                ],
            ],
        ])),
    ...
    expect($result)->toBe('Generated content from MoonShot AI');
});
